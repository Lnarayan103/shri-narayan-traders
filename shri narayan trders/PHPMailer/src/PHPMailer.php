...
class PHPMailer
{
        ...
        switch ($patternselect) {
            ...
            case 'pcre8':
                ...
                return (bool) preg_match(
                    '/^(?!(?>[^\"\\\\]|\\[ -~]){255,})(?!(?>[^\"\\\\]|\\[ -~]){65,}@)' .
                    '((?>((?>[\x0D\x0A]?[\t ])+|([\t ]*\x0D\x0A)?[\t ]+)?(\((?>[^()\\\\]|\\[ -~]|(?3))*\)(?2))|(?2))?)' .
                    '([!#-\'*+\/-9=?^-~-]+|\"(?>((?2)[\x01-\x08\x0B\x0C\x0E-!#-\[\]-\x7F]|\\[ -~]))*' .
    ...
    public function createBody()
    {
        ...
        if (static::ENCODING_BASE64 !== $bodyEncoding && static::hasLineLongerThanMax($this->Body)) {
    ...
    private function getSmtpErrorMessage($base_key)
    {
        ...
        if (!empty($error['error'])) {
            ...
            if (!empty($error['smtp_code'])) {
                $message .= ' ' . $error['smtp_code'];
            }
            if (!empty($error['smtp_code_ex'])) {
                $message .= ' ' . $error['smtp_code_ex'];
            }
        }
    ...
    public static function mb_pathinfo($path, $options = null)
    {
        ...
        $pathinfo = [];
        if (preg_match('#^(.*?)[\\\\\/]*(([^/\\\\]*?)(\.([^.\\\\\/\s]+?)|))[\\\\\/.]*$#m', $path, $pathinfo)) {
